...
QString TryConvertUrlToLocal(QString url) {
	...
	auto subdomainMatch = regex_match(u"^(https?://)?([a-zA-Z0-9\\_]+)\\.(t\\.me|opengra\\.me)(/\\d+)?/?(\\?.+)?"_q, url, matchOptions);
	if (subdomainMatch) {
		...
		if (name.size() > 1 && name != "www") {
			const auto result = TryConvertUrlToLocal(
				...
				+ subdomainMatch->captured(5));
			return result.startsWith("tg://resolve?domain=")
				? result
				: url;
		}
	}
	auto telegramMeMatch = regex_match(u"^(https?://)?(www\\.)?(telegram\\.(me|dog)|t\\.me|opengra\\.me)/(.+)$"_q, url, matchOptions);
	if (telegramMeMatch) {
